...
from prophetcore.mixins import BaseRequiredMixin, get_user_config
from prophetcore.models import Configure
...
class ListModelView(BaseRequiredMixin, ListView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        redirect_to = reverse_lazy('prophetcore:list', args=[self.model_name])
        if action == 'config':
        ...
        if not objects.exists() and action != 'config':
            ...
            try:
                current_action = import_string(
                    'prophetcore.actions.{}'.format(action))
