...
from prophetcore.lib.utils import get_query_string, get_content_type_for_model
from prophetcore.models import Configure, Idc


def construct_menus():
    model_names = []
    for app in apps.get_app_config('prophetcore').get_models():
        opts = app._meta
...
class BaseRequiredMixin(LoginRequiredMixin):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        self.title = "{} 先知漏洞挖掘系统".format(onidc.name)
        if model:
            try:
                self.model = apps.get_model('prophetcore', model.lower())
...
class PostRedirect(object):
    ...
    def get_success_url(self):
        if '_addanother' in self.request.POST:
            url = reverse_lazy('prophetcore:new', kwargs={'model': self.model_name})
            ...
            success_url = reverse_lazy('prophetcore:detail', kwargs=kwargs)
        elif '_last' in self.request.POST:
            referrer = self.request.META.get('HTTP_REFERER', None)
            success_url = referrer
        else:
            kwargs = {'model': self.model_name}
            success_url = reverse_lazy('prophetcore:list', kwargs=kwargs)
        return success_url
